      ...
      <div className="bg-white dark:bg-slate-950 p-6 md:p-8 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="flex items-center gap-4 w-full">
          <div className="h-16 w-16 rounded-2xl overflow-hidden bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-center shrink-0">
            <img 
              src={`https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(userName || "CAT Aspirant")}`} 
              alt="Avatar" 
            ...
            {isEditing ? (
              <div className="flex items-center gap-2 flex-wrap">
                <input
                  ...
                  disabled={saving}
                  className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-3 py-1.5 rounded-xl text-sm font-semibold text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 max-w-xs"
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-xl text-xs font-bold transition-colors"
                >
                  {saving ? "Saving..." : "Save"}
                </button>
                <button
                  ...
                  disabled={saving}
                  className="px-3 py-1.5 bg-slate-100 dark:bg-slate-850 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-650 dark:text-slate-350 rounded-xl text-xs font-bold transition-colors"
                >
                  Cancel
                </button>
              </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-xl font-black">{userName || "Aspirant Student"}</h1>
                <button
                  onClick={handleEdit}
                  className="p-1 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/20 rounded transition-all"
                  title="Edit Name"
                >
        ...
        <div className="flex items-center gap-4 py-3 px-6 rounded-2xl bg-orange-50 dark:bg-orange-950/20 border border-orange-500/10 text-orange-700 dark:text-orange-400 shrink-0">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider text-orange-600 dark:text-orange-500 mt-1">Streaks Active</p>
          </div>
        </div>
      </div>
